...
{
	_("{sibling = ");
	print_Bar(FooBar_sibling(foobar));
	size_t name_len;
...
{
	FooBarContainer container = nbuf_get_FooBarContainer(buf);
...
void test_write(struct nbuf_buffer *buf)
{
	nbuf_clear(buf);
	const int veclen = 3;
	FooBarContainer container = nbuf_init_FooBarContainer(buf);
